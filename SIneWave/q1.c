...
float points = 1000; //no of samples in one time period
...
int main(int c,char *argv[]){
    ...
    double INC = (double)(1.0/((FREQ+30)*points)); //calculate the time increments so as to have point number of samples in one time period. ie 1/(f*points)
    
    gensine(); //puplate the sine lookup table
    
    for(double t=0;t<=0.033;t+=INC){ // time starts from t=0 till t=0.033 which is one time period of the 30Hz sine wave. Each increment is such that there are "points" no of samples in a time period.
